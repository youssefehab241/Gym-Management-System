...
namespace GymManagementSystem
{
    public partial class MembersForm : Form
    {
        ...
        private void SetupUI()
        {
            ...
            txtMemberID = MakeTextBox(leftX + lw + 5, y, iw); this.Controls.Add(txtMemberID);
            ...
            txtAge = MakeTextBox(leftX + lw + 5, y + h * 3, iw); this.Controls.Add(txtAge);
            ...
            txtTrainerID = MakeTextBox(rightX + lw + 5, y, iw); this.Controls.Add(txtTrainerID);
            ...
            txtGoal = MakeTextBox(rightX + lw + 5, y + h * 2, iw); this.Controls.Add(txtGoal);
        ...
        {
            txtMemberID.Text = ""; txtFName.Text = ""; txtLName.Text = "";
            txtAge.Text = ""; txtTrainerID.Text = ""; txtGoal.Text = "";
            dtpJoinDate.Value = DateTime.Now; dtpSessionTime.Value = DateTime.Now;
        }
        ...
        private void btnExecute_Click(object sender, EventArgs e)
        {
            ...
            switch (cmbCommands.SelectedItem.ToString())
            {
                ...
                case "Update Member": UpdateMember(); break;
                case "Delete Member": DeleteMember(); break;
            }
        }
        ...
        private void Search()
        {
            try
            {
                ...
                if (txtMemberID.Text != "") { q += " AND Member_ID = @ID"; cmd.Parameters.AddWithValue("@ID", int.Parse(txtMemberID.Text)); }
                if (txtFName.Text != "") { q += " AND F_Name LIKE @FN"; cmd.Parameters.AddWithValue("@FN", "%" + txtFName.Text + "%"); }
                if (txtLName.Text != "") { q += " AND L_Name LIKE @LN"; cmd.Parameters.AddWithValue("@LN", "%" + txtLName.Text + "%"); }
        ...
        private void AddMember()
        {
            ...
            { MessageBox.Show("Fill ID, Name, and Age."); return; }
            try
            {
                SqlConnection con = new SqlConnection(connectionString);
                con.Open();
        ...
        {
            if (txtMemberID.Text == "") { MessageBox.Show("Enter Member ID."); return; }
            if (MessageBox.Show("Delete this member and all related data?", "Confirm", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
            try
            {
                SqlConnection con = new SqlConnection(connectionString);
                con.Open();
                ...
                cmd3.Parameters.AddWithValue("@ID", id); cmd3.ExecuteNonQuery();

                SqlCommand cmd4 = new SqlCommand("DELETE FROM Member WHERE Member_ID = @ID", con);
                cmd4.Parameters.AddWithValue("@ID", id);
                if (cmd4.ExecuteNonQuery() > 0) MessageBox.Show("Deleted.");
                else MessageBox.Show("Not found.");
                con.Close();
                ViewAll();
            }
        ...
        {
            if (txtMemberID.Text == "") { MessageBox.Show("Enter Member ID first."); return; }
